...
# --- Model Definitions ---
lm_raw <- function(d) lm(Skew_Standardized ~ poly(St, 2) + Re * Fr, data = data_train)
lm_spline <- function(d) lm(Skew_Standardized ~ bs(St, df = 4) + Re * Fr, data = data_train)
gam_auto <- function(d) gam(Skew_Standardized ~ s(St) + Re * Fr, data = data_train)

# --- Cross Validation ---
results <- tibble(
  Model = c("lm_raw", "lm_spline", "gam_auto"),
  MSE = c(cv_error(lm_raw), cv_error(lm_spline), cv_error(gam_auto))
)
print(results)
...
best_model <- switch(
  ...
  gam_auto = gam_auto(data_train)
)
summary(best_model)

# --- Box–Cox Transformation on Best Model ---
bc <- boxcox(best_model, lambda = seq(-2, 2, 0.1))
lambda_opt <- bc$x[which.max(bc$y)]
cat("Optimal lambda from Box-Cox:", lambda_opt, "\n")

# Transform response if beneficial
if (abs(lambda_opt) < 0.1) {
  data_train <- data_train %>%
    mutate(Skew_Transformed = log(Skew_Standardized - min(Skew_Standardized) + 1))
} else {
  data_train <- data_train %>%
    mutate(Skew_Transformed = ((Skew_Standardized - min(Skew_Standardized) + 1)^lambda_opt - 1) / lambda_opt)
}

# --- Refit Models with Transformed Response ---
lm_raw_bc <- function(d) lm(Skew_Transformed ~ poly(St, 2) + Re * Fr, data = d)
lm_spline_bc <- function(d) lm(Skew_Transformed ~ bs(St, df = 4) + Re * Fr, data = d)
gam_auto_bc <- function(d) gam(Skew_Transformed ~ s(St) + Re * Fr, data = d)

# --- CV for Transformed Response ---
results_bc <- tibble(
  Model = c("lm_raw_bc", "lm_spline_bc", "gam_auto_bc"),
  MSE = c(cv_error(lm_raw_bc), cv_error(lm_spline_bc), cv_error(gam_auto_bc))
)
print(results_bc)
...
results_compare <- tibble(
  Model = results$Model,
  MSE_Original = results$MSE,
  MSE_BoxCox = results_bc$MSE
)
print(results_compare)

# --- Final Model after Box–Cox ---
best_name_bc <- results_bc$Model[which.min(results_bc$MSE)]
cat("Best model after Box–Cox:", best_name_bc, "\n")

best_model_bc <- switch(
  best_name_bc,
  lm_raw_bc = lm_raw_bc(data_train),
  lm_spline_bc = lm_spline_bc(data_train),
  gam_auto_bc = gam_auto_bc(data_train)
)

# --- Diagnostics ---
par(mfrow = c(2, 2))
if (best_name_bc == "gam_auto_bc") {
  gam.check(best_model_bc)
  plot(best_model_bc, shade = TRUE)
} else {
  plot(best_model_bc)
}
