...
#We standardized skewness to make it dimensionless, comparable across experimental settings, and to stabilize variance
...
#Difference in rmse between poly 2 and 3 for lm_raw is marginal so we’re sticking with 2 despite anova choosing 3 
...
gam_auto <- function(d) gam(Skew_Standardized ~ s(St) + Re * Fr, data = d)
...
# Plots
par(mfrow = c(2, 2))
if (best_name == "gam_auto") {
  gam.check(best_model)
  plot(best_model, shade = TRUE)
} else {
  plot(best_model)
}
