...
# GLM
```{r}
glm_moment1 <-glm(R_moment_1 ~ poly(St, 2) * Fr + Re:Fr, data = data_train, family = Gamma(link = "log"))
summary(glm_moment1)
par(mfrow = c(2,2))
plot(glm_moment1)
```
...
```{r}
library(splines)
glm_moment2_spline <- glm(R_moment_2 ~ ns(St, 3) + Re * Fr,
                          data = data_train,
                          family = Gamma(link = "log"))
par(mfrow = c(2,2))
plot(glm_moment2_spline)
```
```{r}
