...
```
```{r}
glm_log <- glm(R_moment_1 ~ St * Fr + Re,
               data = data_train,
               family = Gamma(link = "log"))

glm_inv <- glm(R_moment_1 ~ St * Fr + Re,
               data = data_train,
               family = Gamma(link = "inverse"))

glm_id <- glm(R_moment_1 ~ St * Fr + Re,
              data = data_train,
              family = Gamma(link = "identity"))

# Compare fits
AIC(glm_log, glm_inv, glm_id)
...
# make sure Fr and Re are factors (so levels are preserved)
data_train$Fr <- if (!is.factor(data_train$Fr)) factor(data_train$Fr) else data_train$Fr
data_train$Re <- if (!is.factor(data_train$Re)) factor(data_train$Re) else data_train$Re

# create grid of predictor values
st_seq <- seq(min(data_train$St, na.rm = TRUE), max(data_train$St, na.rm = TRUE), length.out = 150)
fr_levels <- levels(data_train$Fr)
re_levels <- levels(data_train$Re)
...
# ensure factor levels match training data before predicting
newdata$Fr <- factor(newdata$Fr, levels = levels(data_train$Fr))
newdata$Re <- factor(newdata$Re, levels = levels(data_train$Re))

# get predictions on the link scale (log) with se.fit, then convert to response scale
pred_link <- predict(glm_moment1, newdata = newdata, type = "link", se.fit = TRUE)

# combine into newdata
newdata$fit_link <- pred_link$fit
newdata$se_link  <- pred_link$se.fit

# transform to response scale (Gamma with log link → inverse link is exp)
newdata$pred  <- exp(newdata$fit_link)
newdata$pred_lwr <- exp(newdata$fit_link - 1.96 * newdata$se_link)
newdata$pred_upr <- exp(newdata$fit_link + 1.96 * newdata$se_link)

# optional: if you prefer plotting on the log scale, use fit_link and exp transforms accordingly

# Plot: predicted mean vs St, colored by Fr, faceted by Re
p <- ggplot(newdata, aes(x = St, y = pred, color = Fr, fill = Fr, group = Fr)) +
  geom_ribbon(aes(ymin = pred_lwr, ymax = pred_upr), alpha = 0.15, color = NA) +
  geom_line(size = 1.05) +
  facet_wrap(~ Re, ncol = length(re_levels)) +
  labs(
    title = "Predicted mean cluster size vs St (by Fr and Re)",
    subtitle = "Predictions from glm_moment1 (Gamma family, log link)",
    x = "Stokes number (St)",
...
ggsave("predicted_mean_by_St_Fr_Re.png", p, width = 10, height = 4 + length(re_levels), dpi = 200)
```
```{r}
