...
# remove 2nd level of polynomial interaction
```{r}
St_poly <- poly(data_train$St, 2)
data_train$St1 <- St_poly[,1]   
data_train$St2 <- St_poly[,2]  

simple_adj_lm_moment1 <- lm(log(R_moment_1) ~ St1 + St2 + St1:Fr + Re * Fr, data = data_train)
...
# GLM
```{r}
glm_moment1 <-glm(R_moment_1 ~ St1 + St2 + Re * Fr, data = data_train, family = Gamma(link = "log"))
summary(glm_moment1)
par(mfrow = c(2,2))
plot(glm_moment1)
```
