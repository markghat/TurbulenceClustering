...
#ANOVA to determine order of polynomial #??Should I include the interaction effect even though it's less interpretable?
...
# ---- MODEL 3: 5-fold CV for Skewness (Central Moment 3) ----
...
# 1️⃣ Compute central moments first (if not already computed)
...
data_train <- data_train %>% 
  mutate(
    skew = central_m3 / (central_m2^(3/2)),  # standardized skew
    # If you prefer raw skewness instead, use: skew = central_m3
    St = as.numeric(St),
    Re = as.numeric(Re),
    Fr = as.numeric(Fr)
  ) %>%
  filter(!is.na(skew), is.finite(skew))  # remove NaN or Inf

# 3️⃣ Build folds
folds <- tryCatch(
  createFolds(data_train$skew, k = 5, list = TRUE),
  error = function(e) {
    split(sample(1:nrow(data_train)), rep(1:5, length.out = nrow(data_train)))
  }
)

# 4️⃣ Cross-validation helper
cv_error <- function(fit_func) {
  sapply(1:5, function(i) {
    train_data <- data_train[-folds[[i]], ]
    test_data  <- data_train[folds[[i]], ]
    model <- tryCatch(fit_func(train_data), error = function(e) NULL)
    if (is.null(model)) return(Inf)
    preds <- predict(model, newdata = test_data, type = "response")
    mean((test_data$skew - preds)^2, na.rm = TRUE)
  }) |> mean()
}

# 5️⃣ Candidate models
lm_raw <- function(d) lm(skew ~ poly(St,2) + Re * Fr, data = d)

lm_spline <- function(d) lm(skew ~ bs(St, df = 4) + Re * Fr, data = d)

gam_gamma <- function(d) gam(skew ~ s(St, k = 4, bs = "cr") + Re * Fr,
                             data = d, family = gaussian())

gam_ti <- function(d) gam(skew ~ te(St, Re, Fr, k = c(3,3,3)),
                          data = d, family = gaussian())

# 6️⃣ Run everything
results <- data.frame(
  Model = c("lm_raw", "lm_spline", "gam_gamma", "gam_ti"),
  MSE = c(cv_error(lm_raw), cv_error(lm_spline), cv_error(gam_gamma), cv_error(gam_ti))
)
